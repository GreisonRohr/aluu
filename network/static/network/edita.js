      ...
      element.addEventListener("change", event => {
        if (event.target.files[0]) {
          ...
          if (event.target.id == 'profile') {
            ...
            event.target.parentElement.querySelector('.custom-file-label').innerHTML = `<span style="color: #6c757d;">Escolha a foto da capa<span>`;
          }
        }
      });
    });
  
    document.querySelectorAll('.role').forEach(input => {
